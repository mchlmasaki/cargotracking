                      ...
                      @foreach($shipments as $shipment)
                        ...
                        <td>{{ $shipment->id }}</td>
                          ...
                          <td>
                            ...
                            $status=$shipment->status;
                            if($status=='Approved'){?>
                              <button data-toggle="modal" href="#modalDefault" onclick="return editUser('{{ $shipment->id }}','{{ $shipment->sender_name }}','{{ $shipment->sender_contact }}','{{ $shipment->weight }}','{{ $shipment->cons_no }}','{{ $shipment->from_location }}','{{ $shipment->to_location }}','{{ $shipment->status }}')" class="btn btn-success btn-sm "> Update </button><?php
              ...
              <!-- Small modal -->
                  ...
                  <div class="modal fade" id="modalDefault" tabindex="-1" role="dialog" aria-hidden="true">
                        ...
                        <div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
                            <label for="number_plate" class="col-md-4 control-label">Assign/Select container</label>

                            <div class="col-md-8">
                            
                          <select class="form-control" id="container_number" type="text" class="form-control" name="container_number" >
                          @foreach($containers as $container)
                          <option value="{{ $container->container_number }}">{{ $container->name }}</option>
                            @endforeach
                          </select>
...
<script type="text/javascript">
  function editUser(id,sender_name,sender_contact,weight,cons_no,from_location,to_location,status)
  {
