  ...
  $result = mysqli_query($conn,"SELECT * FROM employeeoffices Where user_id =$empl_id ");
    while($row = mysqli_fetch_array($result)){
                  ...
                  ?>
                          ...
                          <td><button data-toggle="modal" href="#updateEmployee" onclick="return editUser('{{ $user->id }}','{{ $user->name }}','{{ $user->contact }}','{{ $user->email }}','{{ $user->role }}')" class="btn btn-success btn-xs "><i class="fa fa-edit"></i> Update </button> 
                          <a class="btn btn-danger btn-xs waves-effect waves-light remove-record" onclick="return confirmDelete('{{ $user->id }}')" data-toggle="modal" data-url="{!! URL::to('Admin/employees/delete', $user->id) !!}" data-id="{{$user->id}}" data-target="#confirmDelete"><i class="fa fa-trash"></i>&nbsp;Delete</a>
