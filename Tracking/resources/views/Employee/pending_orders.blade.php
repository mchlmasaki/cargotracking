                    ...
                    <div class="col-md-12">
                      ...
                      <div class="" role="tabpanel" data-example-id="togglable-tabs">
                        <ul id="myTab" class="nav nav-tabs bar_tabs" role="tablist">
                          ...
                          <li role="presentation" class="active"><a href="{{  url('Employee/pending_orders') }}"  aria-expanded="true">Pending</a>
                          </li>
                  ...
                  <table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
                      ...
                      @foreach($shipments as $shipment)
                        ...
                        <td>{{ date('M d, Y', strtotime( $shipment->created_at )) }}</td>
                          ...
                          <td><button class="btn btn-info btn-sm">{{ $shipment->status }}</button></td>
                          <td><a href="{{ url('Employee/shipmentdetails/'.$shipment->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-folder"></i> View </a><a href="{{ url('Employee/manage_shipment/'.$shipment->id) }}" class="btn btn-primary btn-sm"> Manage </a>
                          <td><a href="{{ url('Employee/shipmentdetails/'.$shipment->id) }}" class="btn btn-warning btn-sm">Cancel </a>
                          <a class="btn btn-danger btn-sm waves-effect waves-light remove-record" onclick="return confirmDelete('{{ $shipment->id }}')" data-toggle="modal" data-url="{!! URL::to('Employee/pending_orders/delete', $shipment->id) !!}" data-id="{{$shipment->id}}" data-target="#confirmDelete"><i class="fa fa-trash"></i>&nbsp;Delete</a>
                          </td>
...
<form action="{{ URL::to('Employee/pending_orders/delete/') }}" method="POST" class="remove-record-model">
{{csrf_field()}}
{{method_field('delete')}}
