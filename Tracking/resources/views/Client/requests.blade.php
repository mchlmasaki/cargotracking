            ...
            <div class="row">
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
                      ...
                      @foreach($shipments as $shipment)
                        ...
                        <td>{{ $shipment->sender_address }}</td>
                          ...
                          <td><a href="{{ url('Client/shipmentdetails/'.$shipment->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-folder"></i> View </a>
                            <?php
                          $status=$shipment->status ;

                  if ($status=='Approved' || $status=='Loaded' || $status=='In Transit' || $status=='Delivered' || $status=='Completed' || $status=='Cancelled'){

                          }else{?>
                          <button data-toggle="modal" href="#modalDefault" onclick="return editUser('{{ $shipment->id }}','{{ $shipment->sender_name }}','{{ $shipment->sender_contact }}','{{ $shipment->from_location }}','{{ $shipment->sender_address }}','{{ $shipment->receiver_name }}','{{ $shipment->receiver_contact }}','{{ $shipment->to_location }}','{{ $shipment->receiver_address }}','{{ $shipment->type_ofshipment }}','{{ $shipment->product_name }}','{{ $shipment->qty }}','{{ $shipment->weight }}','{{ $shipment->description }}','{{ $shipment->mode }}')" class="btn btn-success btn-sm "><i class="fa fa-pencil"></i> Edit </button>
                  ...
                  if ($status=='Approved' || $status=='Pending' || $status=='Delivered' || $status=='Completed' || $status=='Cancelled'){
                    ?>
